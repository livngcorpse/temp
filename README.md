...
### Is this a fully Dockerised application?

The **backend is fully containerised** and production-ready via Docker.

The **frontend is not containerised** — it's a Next.js app deployed as a serverless build on Vercel, which is the recommended and most cost-effective approach. If you want to run the frontend in Docker too, you can add a `frontend/Dockerfile` like this:
...
And enable `output: 'standalone'` in `frontend/next.config.js`:
